// src/components/UserWithdrawRequests.js
import React, { useEffect, useState } from "react";

const UserWithdrawRequests = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            const res = await fetch(`${API}/api/deposits/get-withdraw-requests`, {
                headers: {
    ...
    const handleAction = async (id, action) => {
        try {
            ...
            const res = await fetch(`${API}/api/deposits/approve-withdraw-request/${id}/${action}`, {
                method: "PUT",
                headers: {
            ...
            const data = await res.json();
            if (res.ok) {
                alert(`Withdraw ${action}ed successfully`);
    ...
    return (
        <div className="p-6 text-gray-200 bg-gray-900 min-h-screen w-[100vw]">
            ...
            <h2 className="text-xl font-semibold text-cyan-400 mb-4">User Withdraw Requests</h2>
                        ...
                        {requests.length > 0 ? (
                            requests.map((req) => (
                                <tr key={req._id} className="border-t border-gray-700 hover:bg-gray-800/50">
                                    ...
                                    <td className="p-3 text-yellow-400 font-medium">₹{req.amount}</td>
...
export default UserWithdrawRequests;
